...
void main(int comando, char* comando_p[])
{
	...
	while (1) {
		...
		if (fgets(Messages_queue.data, total_msg_size, stdin) == NULL)
		{
			printf("\nERROR");
		...
		msgsnd(interfaz1, &Messages_queue, sizeof(Messages_queue) - sizeof(long), 0);
		printf("\nEl mensaje se ha enviado por la cola!");

		//Para recibir el mensaje de nuevo

			printf("\nEsperando mensaje...");
			msgrcv(interfaz1, (Messages_queue*)&Messages_queue, sizeof(Messages_queue) - sizeof(long), (long)getpid(), 0);
		...
		//Messages_queue.queue_direction = mesg_cola.cola_destino;
